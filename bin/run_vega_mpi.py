...
if __name__ == "__main__":
    pars = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter, description="Run Vega in parallel."
    )
        ...
        if sampler.use_mpi:
            ...
            with MPIPool(mpi_comm) as pool:
                pocomc_sampler = pocomc.Sampler(
                    ...
                    log_lik,
                    pool=pool,
                    output_dir=sampler.pocomc_output,
                    dynamic=sampler.dynamic,
                    precondition=sampler.precondition,
                    n_effective=sampler.n_effective,
                    n_active=sampler.n_active,
                )
                pocomc_sampler.run(
