...
def main(ini, output_path, fid_H0, fid_Ok, fid_wl, z_ref, no_extrap, match_chain=None):
    ...
    sb1_rmin = 50.0 * coef_Planck2015
    sb1_rmax = 82.0 * coef_Planck2015
    sb2_rmin = 150.0 * coef_Planck2015
    sb2_rmax = 190.0 * coef_Planck2015
    xi = pk_to_xi(k, pk)
    r = np.logspace(-7.0, 3.5, 10000)
    xi = xi(r)
    ...
    sigma[(r >= sb1_rmin - 2.0) & (r < sb1_rmin + 2.0)] = 1.0e-6
    sigma[(r >= sb2_rmax - 2.0) & (r < sb2_rmax + 2.0)] = 1.0e-6
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--ini", type=str, required=True, help="Input config file for CAMB")
