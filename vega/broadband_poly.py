...
class BroadbandPolynomials:
    ...
    def __init__(self, bb_input, cf_name, model_coordinates, dist_model_coordinates):
        ...
        self.bb_terms = {"pre-add": [], "pre-mul": [], "post-add": [], "post-mul": []}
    ...
    def _compute_broadband_sky(bb_term_name, params, coordinates):
        ...
        corr = scale / (sigma * np.sqrt(2.0 * np.pi))
        corr *= np.exp(-0.5 * (coordinates.rt_grid / sigma) ** 2)
        w = (coordinates.rp_grid >= 0.0) & (coordinates.rp_grid < coordinates.rp_binsize)
        corr[~w] = 0.0
    ...
    def _compute_broadband(bb_term, params, coordinates):
        ...
        corr = (
            bb_params[None, :, :]
            * r1[:, None, None] ** r1_powers[None, :, None]
            * r2[:, None, None] ** r2_powers[None, None, :]
        ).sum(axis=(1, 2))
