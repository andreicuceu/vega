...
def plot_wedges(models, covariance, multi_model=False, labels=None, data=None, cross=False):
    ...
    mus = np.array([0.0, 0.5, 0.8, 0.95, 1.0])
    mu_zip = zip(mus[:-1], mus[1:])

    for i, mu in enumerate(mu_zip):
        if not cross:
            wedge_obj = Wedge(
                mu=mu, rp=(0.0, 200.0, 50), rt=(0.0, 200.0, 50), r=(0.0, 200.0, 50), abs_mu=True
            )
        else:
            wedge_obj = Wedge(
                mu=mu, rp=(-200.0, 200.0, 100), rt=(0.0, 200.0, 50), r=(0.0, 200.0, 50), abs_mu=True
            )
...
def plot_shells(vega, model, angle_var="theta", rs=(30, 40, 50, 60, 70), corr="lyaxlya"):
    ...
    for i, r in enumerate(r_zip):
        ...
        axs[0, i // 2].errorbar(
            mud,
            dd * 1e3,
            yerr=np.sqrt(cd.diagonal()) * 1e3,
            fmt=fmts[i],
            c=colors[i],
            capsize=2,
            label=r"$r \in [{}, {}]$ Mpc/h".format(r[0], r[1]),
        )

        mu, d, _ = wedge_obj(model[corr][mask], covariance=vega.data[corr].cov_mat)
        axs[1, i // 2].errorbar(
            mud,
            (dd - d) / np.sqrt(cd.diagonal()),
            yerr=np.ones_like(d),
            fmt=fmts[i],
            c=colors[i],
            capsize=2,
