    ...
    for name in params:
        if name in latex_full:
            ...
            if tracer is not None and comp_par is not None:
                ...
                latex_names[comp_name] = composites[comp_par] + latex_comp[tracer] + r"}"
            elif comp_par is not None:
                print(
                    "Warning: No latex name found for tracer: %s. If you"
                    " want plots to work well, add a latex name to"
                    " latex_composite.txt" % name[len(comp_par) + 1 :]
                )
                latex_names[name] = composites[comp_par] + name[len(comp_par) + 1 :] + r"}"
            else:
                print(
                    "Warning! No latex name found for %s. Add the latex"
