from . import broadband_poly, metals, pktoxi, power_spectrum
...
class Model:
    ...
    def __init__(self, corr_item, fiducial, scale_params, data=None):
        ...
        data_is_direct_multipoles = (self._data is not None) and self._data.is_direct_multipoles
        ...
        if "broadband" in corr_item.config:
            self.broadband = broadband_poly.BroadbandPolynomials(
                corr_item.config["broadband"],
                corr_item.name,
                corr_item.model_coordinates,
                corr_item.dist_model_coordinates,
            )

        # Initialize main Power Spectrum object
        self.Pk_core = power_spectrum.PowerSpectrum(
            corr_item.config["model"],
            fiducial,
            corr_item.tracer1,
            corr_item.tracer2,
            corr_item.name,
        ...
        involves_qso = (
            corr_item.tracer1["type"] == "discrete" or corr_item.tracer2["type"] == "discrete"
        )
    ...
    def compute_from_input_pk_model(self, pars, k_grid, muk_grid, pk_model, no_metals=False):
        ...
        assert pk_model.shape[0] == muk_grid.size, "Mismatch between pk_model and muk_grid sizes"

        if len(muk_grid.shape) == 1:
            muk_grid = muk_grid[:, None]

        self._pktoxi_alternative = pktoxi.PktoXi(k_grid, muk_grid, self._corr_item.config["model"])
